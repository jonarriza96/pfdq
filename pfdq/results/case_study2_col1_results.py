...
ax.plot(
    thf5_modified, thf_d, "--", color="magenta"
)  # , alpha=0.75,linewidth=lw)  # thp
ax.plot(
    thf5, thDf5, color="blueviolet", linewidth=2
)  # , alpha=0.75,linewidth=lw)  # thp

ax.plot(
    thf5_modified, ths_d, "--", color="limegreen"
)  # , alpha=0.75,linewidth=lw)  # thp
ax.plot(
    ths5, thDs5, color="forestgreen", linewidth=2
)  # , alpha=0.85,linewidth=lw)  # thm

ax.plot(
    thf5_modified, thv_d, "--", color="goldenrod"
)  # , alpha=0.75,linewidth=lw)  # thp
ax.plot(
    thv5, thDv5, color="darkorange", linewidth=2
)  # , alpha=0.75,linewidth=lw)  # thc
...
plt.setp(
    ax, xticks=[0.65, 0.85, 1.0], xlim=[0.62, 1.01]
)  # , xticklabels=['a', 'b', 'c']
# for AX in ax:
#     AX.grid(axis='x')
ax1, ax2 = ax
